...
# this doesn't work
# f2 <- "https://raw.githubusercontent.com/ropensci/tabulizer/main/inst/examples/data.pdf"
...
extract_tables(f, method = "decide")
```
...
extract_tables(f, pages = 2, method = "lattice")
```

```{r}
# Extract tables by using "stream" method
# extract_tables(f2, pages = 2, method = "stream")
extract_tables(f, pages = 2, method = "stream")
```
